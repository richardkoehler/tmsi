...
class Poly5_to_EDF_Converter:
    def __init__(self, batch=None, filename=None, foldername=None, f_c=0.1):
        ...
        if not batch:
            ...
            n_poly5 = 0
            conversion_files = []
            for root, _dirs, files in os.walk(foldername):
                for file in files:
    ...
    def _filter_data(self):
        ...
        if not isinstance(self.f_c, list):
            print(
                "Data is high-pass filtered with cut-off frequency ",
                self.f_c,
                "Hz",
            )
    ...
    def _write_edf_meta_data(self):
        ...
        self.hdl = EDFwriter(
            self.edf_filename,
            EDFwriter.EDFLIB_FILETYPE_EDFPLUS,
            self.n_signals,
        )
            ...
            if max(self.data.samples[chan, :]) == min(self.data.samples[chan, :]):
                ...
                if not isinstance(self.f_c, list):
                    ...
                    self.hdl.setPreFilter(
                        chan,
                        "HP:" + str(self.f_c[0]) + "Hz LP:" + str(self.f_c[1]) + "Hz",
                    )
