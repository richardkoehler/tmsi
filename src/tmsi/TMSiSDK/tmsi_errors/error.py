...
class TMSiError(Exception):
    ...
    def __str__(self):
        if self.dll_error_code:
            return f"Error code {self.error_code.value}: {SdkErrorLookupTable(str(self.error_code.value))} \nDLL Error code {self.dll_error_code}: {DeviceErrorLookupTable(self.dll_error_code)}\n{self.message}"
        return f"Error code {self.error_code.value}: {SdkErrorLookupTable(str(self.error_code.value))}\n{self.message}"
