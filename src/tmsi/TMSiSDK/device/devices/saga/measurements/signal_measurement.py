...
class SignalMeasurement(TMSiMeasurement):
    ...
    def start(self):
        ...
        TMSiLoggerActivity().log(f"{self.get_name()}->>SAGA-SDK: set device sampling request ON")
    ...
    def stop(self):
        ...
        measurement_request.DisableAvrRefCalc = 0
        TMSiLoggerActivity().log(f"{self.get_name()}->>SAGA-SDK: set device sampling request OFF")
        self._dev.set_device_sampling_request(measurement_request)
