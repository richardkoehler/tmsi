...
class ApexDevice(TMSiDevice):
    ...
    def discover(dr_interface: DeviceInterfaceType, num_retries: int = 3):
        ...
        while num_retries > 0:
            ...
            if ret == TMSiDeviceRetVal.TMSiStatusOK:
                    ...
                    if device_list[i].TMSiDeviceID != ApexConst.TMSI_DEVICE_ID_NONE:
                        for ii in range(ApexDevice.__MAX_NUM_DEVICES):
                            if ApexDevice.__device_info_list[ii].get_id() == ApexConst.TMSI_DEVICE_ID_NONE:
                                ...
                                ApexDevice.__device_info_list[ii].set_dongle_serial_number(
                ...
                num_retries -= 1
                TMSiLogger().warning(
                    "Impossible to find devices. Number of retries left: " + str(num_retries)
                )
    ...
    def export_configuration(self, filename: str):
        ...
        if self.__info.get_state() == DeviceState.connected:
            ...
            if self.__config.export_to_xml(filename):
                TMSiLoggerActivity().log("APEX-API->>APEX-SDK: export succeeded")
                return
            else:
    ...
    @LogPerformances
    def get_device_interface_status(self, interface=DeviceInterfaceType.bluetooth) -> TMSiInterfaceStatus:
    ...
    def get_device_list(dr_interface: DeviceInterfaceType) -> list:
        ...
        for idx in range(ApexDevice.__MAX_NUM_DEVICES):
            if (
                ApexDevice.__device_info_list[idx].get_id() != ApexConst.TMSI_DEVICE_ID_NONE
                and ApexDevice.__device_info_list[idx].get_dr_interface() == dr_interface
            ):
                device_list.append(
                    ApexDevice(
    ...
    def set_device_download_file_request(self, file_request: TMSiDevSetCardFileReq):
        ...
        self.__last_error_code = TMSiSetDeviceCardFileRequest(self.__device_handle, pointer(file_request))
        if self.__last_error_code == TMSiDeviceRetVal.TMSiStatusOK:
            return
        else:
            raise TMSiError(TMSiErrorCode.device_error, self.__last_error_code)
    ...
    def set_device_impedance_request(self, measurement_request: TMSiDevImpedanceRequest):
        ...
        if self.__last_error_code == TMSiDeviceRetVal.TMSiStatusOK:
            ...
            TMSiLoggerActivity().log(f"APEX-API->>APEX-SDK: start failed with error {self.__last_error_code}")
            raise TMSiError(TMSiErrorCode.device_error, self.__last_error_code)
    ...
    def set_device_interface(
        ...
        self.__set_device_interface_config(device_interface=device_interface, control=control)
    ...
    def set_device_references(self, list_references: list, list_indices: list):
        ...
        TMSiLoggerActivity().log("APEX-SDK->>APEX-API: set device reference configuration")
        self.__set_device_reference_config(list_references=list_references, list_indices=list_indices)
        return self.get_device_channels()
    ...
    def start_download_file(self, file_id: int, filename: str = None, n_of_samples: int = None):
        ...
        TMSiLoggerActivity().log(f"TMSi-SDK->>{self.__measurement.get_name()}: start")
    ...
    def stop_download_file(self):
        ...
        if self.__info.get_state() != DeviceState.sampling:
            raise TMSiError(error_code=TMSiErrorCode.api_invalid_command)
        TMSiLoggerActivity().log(f"TMSi-SDK->>{self.__measurement.get_name()}: stop")
    ...
    def __initialize():
        try:
            ...
            for _i in range(ApexDevice.__MAX_NUM_DEVICES):
                ApexDevice.__device_info_list.append(ApexInfo())
            for _i in range(ApexDevice.__MAX_NUM_DONGLES):
    ...
    def __download_impedance_report(self, file_id, filename):
        ...
        try:
            with open(f"{filename}.txt", "w") as f:
                f.write(f"{card_file_details.RecFileName.decode()}\n\n")
                f.write("Idx\t\tName\t\tRe\t\t\tIm\n")
                f.write(
                    "\n".join(
                        [
                            f"{i.ChanIdx}\t\t{i.ChanName.decode()}\t\t{i.ImpedanceRe} kOhm\t\t{i.ImpedanceIm} nF"
    ...
    def __get_device_card_recording_config(self):
        ...
        self.__last_error_code = TMSiGetDeviceCardRecordingConfig(self.__device_handle, pointer(config))
        if self.__last_error_code == TMSiDeviceRetVal.TMSiStatusOK:
            return config
        else:
            raise TMSiError(TMSiErrorCode.device_error, self.__last_error_code)
    ...
    def __get_device_card_status(self):
        ...
        self.__last_error_code = TMSiGetDeviceCardStatus(self.__device_handle, pointer(card_status))
        if self.__last_error_code == TMSiDeviceRetVal.TMSiStatusOK:
            return card_status
        else:
            raise TMSiError(TMSiErrorCode.device_error, self.__last_error_code)
    ...
    def __get_device_info_report(self):
        ...
        self.__last_error_code = TMSiGetDeviceInfo(self.__device_handle, pointer(device_info_report))
        if self.__last_error_code == TMSiDeviceRetVal.TMSiStatusOK:
            return device_info_report
        else:
            raise TMSiError(TMSiErrorCode.device_error, self.__last_error_code)
    ...
    def __get_device_power_status(self):
        ...
        self.__last_error_code = TMSiGetDevicePowerStatus(self.__device_handle, pointer(power_status))
        if self.__last_error_code == TMSiDeviceRetVal.TMSiStatusOK:
            return power_status
        else:
            raise TMSiError(TMSiErrorCode.device_error, self.__last_error_code)
    ...
    def __get_device_rtc(self):
        dev_time = TMSiTime()
        self.__last_error_code = TMSiGetDeviceRTC(self.__device_handle, pointer(dev_time))
        if self.__last_error_code == TMSiDeviceRetVal.TMSiStatusOK:
    ...
    def __set_device_card_recording_config(self, config):
        self.__last_error_code = TMSiSetDeviceCardRecordingConfig(self.__device_handle, pointer(config))
        if self.__last_error_code == TMSiDeviceRetVal.TMSiStatusOK:
            return
        else:
            raise TMSiError(TMSiErrorCode.device_error, self.__last_error_code)
    ...
    def __set_device_rtc(self, dt):
        ...
        self.__last_error_code = TMSiSetDeviceRTC(self.__device_handle, pointer(dev_time))
        if self.__last_error_code == TMSiDeviceRetVal.TMSiStatusOK:
            return
        else:
            raise TMSiError(TMSiErrorCode.device_error, self.__last_error_code)
