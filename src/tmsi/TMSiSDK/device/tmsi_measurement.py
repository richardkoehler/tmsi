...
class TMSiMeasurement:
        ...
        for i in range(len(channels)):
            if channels[i].get_channel_format() == 0x0020:
                ...
                if (channels[i].get_channel_type().value == 3) and (
                    channels[i].get_sensor_information() is not None
                ):
                    self._sensor_channels.append(i)
                else:
                    conversion_factor = 10 ** channels[i].get_channel_exp()
                    if conversion_factor not in self._basic_conversion:
    ...
    def set_conversion_pause(self, pause):
        ...
        :type pause: float
        """
        logger().debug(f"conversion pause set to {pause}")
    ...
    def set_sampling_pause(self, pause):
        ...
        :type pause: float
        """
        logger().debug(f"sampling pause set to {pause}")
