...
class ImpedanceMeasurement(TMSiMeasurement):
    ...
    def __init__(self, dev, name="Impedance Measurement"):
        ...
        TMSiLoggerActivity().log(f"TMSi-SDK->>{self.get_name()}: create measurement")
        TMSiLoggerActivity().log(
            f"{self.get_name()}->>{self._dev.get_device_type()}-SDK: GET device channels request"
        )
        channels = self._dev.get_device_active_impedance_channels()
        TMSiLoggerActivity().log(
            f"{self._dev.get_device_type()}-SDK->>{self.get_name()}: GET device channels response"
        )
    ...
    def start(self):
        ...
        TMSiLoggerActivity().log(f"{self.get_name()}->>SAGA-SDK: set device impedance request ON")
    ...
    def stop(self):
        ...
        TMSiLoggerActivity().log(f"{self.get_name()}->>SAGA-SDK: set device impedance request OFF")
        self._dev.set_device_impedance_request(measurement_request)
    ...
    def _sampling_function(self):
        ...
        if ret == TMSiDeviceRetVal.TMSI_OK:
            ...
            TMSiLoggerActivity().log(f"SAGA-SDK->>Sampling Thread: negative response: {ret}")
        if self._download_samples_limit is not None:
            ...
            TMSiLoggerActivity().log(
                f"Sampling Thread->>Sampling Thread: downloaded {self._downloaded_samples} samples of {self._download_samples_limit}"
            )
