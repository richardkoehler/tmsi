...
from ..sample_data_server.event_data import EventData, EventDataConsumer
from ..sample_data_server.sample_data import SampleData, SampleDataConsumer
...
class SampleDataServer(metaclass=Singleton):
            ...
            if self.__consumer_list[i].id == id:
                if not hasattr(self.__consumer_list[i].q, "get_consumer_id"):
                    ...
                    TMSiLoggerActivity().log(
                        f"SDS->>Consumer{self.__consumer_list[i].q.get_consumer_id()}: PUT sample data"
                    )
                    self.__consumer_list[i].q.put(data)
    ...
    def unregister_consumer(self, id: int, q: Queue):
        ...
        num_consumers = len(self.__consumer_list)
        for i in range(num_consumers):
            if self.__consumer_list[i].id == id and self.__consumer_list[i].q == q:
                idx_remove = copy(i)
    ...
    def unregister_event_consumer(self, id: int, q: Queue):
        ...
        num_consumers = len(self.__event_consumer_list)
        for i in range(num_consumers):
            if self.__event_consumer_list[i].id == id and self.__event_consumer_list[i].q == q:
                idx_remove = copy(i)
