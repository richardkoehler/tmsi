...
class DownloadMeasurement(SignalMeasurement):
    ...
    def start(self):
        ...
        TMSiLoggerActivity().log(f"{self.get_name()}->>APEX-SDK: set device download request ON")
    ...
    def stop(self):
        ...
        TMSiLoggerActivity().log(f"{self.get_name()}->>APEX-SDK: set device download request OFF")
        self._dev.set_device_download_file_request(file_request)
