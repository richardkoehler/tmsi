...
class Xdf_Reader:
    ...
    def _add_ch_locations(self, info):
        ...
        chLocs = pd.read_csv(
            join(modules_dir, "TMSiSDK/tmsi_resources/EEGchannelsTMSi3D.txt"),
            sep="\t",
            header=None,
        )
        ...
        for idx, ch in enumerate(info["chs"]):
            try:
                a = [
                    i
                    for i, e in (
                        enumerate(chLocs["eeg_name"].values) or enumerate(chLocs["default_name"].values)
                    )
                    if e == ch["ch_name"]
                ]
