...
class SagaDevice(TMSiDevice):
    ...
    def apply_mask(self, n_channels, masks):
        ...
        if hasattr(self, "__measurement") and hasattr(self.__measurement, "apply_mask"):
    ...
    def download_file_from_device(self, file_id: int, filename: str = None):
        ...
        while True:
            ...
            TMSiLogger().debug(f"downloaded: {percentage:.2f}%")
            if percentage >= 100:
                break
    ...
    def set_device_impedance_request(self, measurement_request: TMSiDevImpReq):
        ...
        if self.__info.get_state() != DeviceState.sampling:
            raise TMSiError(error_code=TMSiErrorCode.api_invalid_command)
        self.__last_error_code = TMSiSetDeviceImpedance(self.__device_handle, pointer(measurement_request))
        if self.__last_error_code == TMSiDeviceRetVal.TMSI_OK:
            return
        else:
            raise TMSiError(
    ...
    @LogPerformances
    def set_device_sampling_config(self, base_sample_rate=None, channel_type=None, channel_divider=1):
    ...
    def set_device_sampling_request(self, measurement_request: TMSiDevSampleReq):
        ...
        self.__last_error_code = TMSiSetDeviceSampling(self.__device_handle, pointer(measurement_request))
        if self.__last_error_code == TMSiDeviceRetVal.TMSI_OK:
            return
        else:
            TMSiLoggerActivity().log(f"SAGA-API->>SAGA-SDK: start failed with error {self.__last_error_code}")
            raise TMSiError(
                error_code=TMSiErrorCode.device_error,
                dll_error_code=self.__last_error_code,
            )
    ...
    def __download_impedance_report(self, file_id, filename):
        ...
        try:
            with open(f"{filename}.txt", "w") as f:
                f.write(f"Recorded Filename: {recording_metadata.RecFileName.decode()}\n\n")
                f.write("Idx\t\tName\t\t\tImpedance\n")
                imp_channels = self.__config.get_channels()
                f.write(
                    "\n".join(
                        [
                            f"{i.ChanNr}\t\t{imp_channels[i.ChanNr].get_channel_name()}\t\t{i.Impedance} kOhm"
                            for i in impedances
                        ]
                    )
                )
    ...
    def __initialize():
        try:
            ...
            for _i in range(SagaDevice.__MAX_NUM_DEVICES):
    ...
    def __get_device_card_recording_config(self):
        ...
        self.__last_error_code = TMSiGetDeviceAmbConfig(self.__device_handle, pointer(config))
        if self.__last_error_code == TMSiDeviceRetVal.TMSI_OK:
            return config
        else:
            raise TMSiError(
    ...
    def __get_device_sensors(self):
        ...
        if self.__last_error_code == TMSiDeviceRetVal.TMSI_OK:
            self.__update_sensor_list(
                device_sensor_list=device_sensor_list,
                sensor_list_len=sensor_list_len,
            )
        else:
            raise TMSiError(
                error_code=TMSiErrorCode.device_error,
    ...
    def __reset_device_card(self):
        self.__set_device_card_recording_config(config=self.__get_device_card_recording_config())
    ...
    def __set_device_channel_sample_rates(self, channel_type, channel_divider):
        for channel in self.__config.get_channels():
            if channel.get_channel_divider() != -1 and (
                ChannelType.all_types == channel_type or channel.get_channel_type() == channel_type
            ):
                channel.set_channel_divider(
    ...
    def __set_device_config(self, base_sample_rate=None):
        ...
        dev_set_config.SetBaseSampleRateHz = (
            self.__config.get_sample_rate() if base_sample_rate is None else base_sample_rate.value
        )
    ...
    def __update_sensor_list(self, device_sensor_list, sensor_list_len):
        ...
        for i in range(sensor_list_len.value):
            ...
            if channel_count > 0:
                if (
                    self.__config.get_channels()[device_sensor_list[i].ChanNr].get_channel_type()
                    == ChannelType.AUX
                ):
                    ...
                    for _j in range(channel_count):
                        ...
                        if struct_id[0] == 0x0000:
                            idx += 2
                            chan_name, unit_name, exp, gain, offset = struct.unpack_from(
                                "<10s10shff",
                ...
                if (
                    ...
                    self.__config.get_channels()[device_sensor_list[i].ChanNr].set_sensor_information(
                        sensor=copy(sensor), bipolar=True
                    )
